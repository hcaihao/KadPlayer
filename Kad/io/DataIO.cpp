...
CKadTag *CDataIO::ReadTag(bool bOptACP)
{
	...
	{
		CString strErr;
		strErr.Format(_T("Invalid Kad tag; type=0x%02x  lenName=%u  name=0x%02x"), byType, uLenName, pcName!=NULL ? (BYTE)pcName[0] : 0);
		TRACE("%s\r\n",strErr);
		delete[] pcName;
		delete pRetVal;
		throw;
...
void CDataIO::WriteTag(const CKadTag* pTag)
{
	...
	{
		CString strErr;
		strErr.Format( _T("Exception in CDataIO:writeTag (IO Error(%i))"), ioe->m_iCause);
		TRACE("%s\r\n",strErr);
		throw ioe;
	}
	catch (...)
	{
		TRACE("Exception in CDataIO:writeTag");
		throw;
	}
}
...
bool CKademlia::InitUnicode(HMODULE hInst ,UINT nResID,LPCWSTR lpType)
{
	bool bResult = false;
	//HRSRC hResInfo = FindResource(hInst, MAKEINTRESOURCE(IDR_WIDECHARMAP1), _T("WIDECHARMAP"));

	HRSRC hResInfo = FindResource(hInst, MAKEINTRESOURCE(nResID), lpType);
