...
void CKademlia::Start(CPrefs *pPrefs)
{
	try
	{
		...
		m_pInstance->m_pRoutingZone = new CRoutingZone(); //CRoutingZone，CRoutingBin和CContact三个类组成了每个节点所了解的联系信息以及由这些联系信息所组成的数据结构。
...
void CKademlia::Process()
{
	...
	{
		//theApp.emuledlg->RefreshUPnP(); //DEL LIPING
	...
	for (EventMap::const_iterator itEventMap = m_mapEvents.begin(); itEventMap != m_mapEvents.end(); ++itEventMap)
	{
		...
		{
			/*
			CRoutingZone中OnSmallTimer()，在此函数体内，当判断联系人为非空时，调用函数 CKademlia::GetUDPListener()->SendMyDetails_KADEMLIA2(KADEMLIA2_HELLO_REQ, 
			pContact->GetIPAddress(), pContact->GetUDPPort());来发送本地节点的一些信息，其中函数的第一个参数是消息的类型， KADEMLIA2_HELLO_REQ表明是Kademlia 2.0网络的加入请求，
			相当于TCP/IP中的ACK，即表明这个消息是用来加入网络的。第二个参数是本地节点的IP，第三个节点是本地节点的端口。
			*/
			pZone->OnSmallTimer();
	...
	if( bUpdateUserFile )
	{
		if( uMaxUsers != m_pInstance->m_pPrefs->GetKademliaUsers())
		{
			...
			//theApp.emuledlg->ShowUserCount();//DEL LIPING
		}
	}
...
uint32 CKademlia::GetPublicIP(bool bIgnoreKadIP) 
{
	m_dwPublicIP = 0;
	if (m_dwPublicIP == 0 && IsConnected() && GetIPAddress() && !bIgnoreKadIP)
		return ntohl(GetIPAddress());
	return m_dwPublicIP;
}
bool  CKademlia::KadIsConnected()
{
