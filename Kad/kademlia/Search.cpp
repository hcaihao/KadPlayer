...
#include "../kademlia/SearchList.h"
#include "../socket/ClientUDPSocket.h"
#include "../comm/log.h"
...
CSearch::~CSearch()
{
	...
	CPartFile* pPartFile=NULL ;//= m_KadManager.pDownLoadQueue->GetFileByKadFileSearchID(GetSearchID());//DEL LIPING
	...
	if (m_uType == NOTES){
		CAbstractFile* pAbstractFile = m_KadManager.pKnownFileList->FindKnownFileByID(CUInt128(GetTarget().GetData()).GetData());//
		if (pAbstractFile != NULL)
			pAbstractFile->SetKadCommentSearchRunning(false);

		pAbstractFile= m_KadManager.pDownLoadQueue->GetFileByID(CUInt128(GetTarget().GetData()).GetData());//
		if (pAbstractFile != NULL)
			pAbstractFile->SetKadCommentSearchRunning(false);

		theApp.m_KadImpl.m_SearchList.SetNotesSearchStatus(CUInt128(GetTarget().GetData()).GetData(), false); //
	}
#endif
...
void CSearch::Go()
{
	...
	if (m_mapPossible.empty())
	{
		...
		//theApp.emuledlg->kademliawnd->UpdateSearchGraph(m_pLookupHistory);//DEL LIPING
	}
	if (!m_mapPossible.empty())
	{
	...
	//theApp.emuledlg->kademliawnd->searchList->SearchRef(this);//DEL LIPING
		...
		case FILE:
			{
				...
				if (pFromContact->GetVersion() >= 3/*47b*/)
				{
					...
					CKnownFile* pFile = m_KadManager.pDownLoadQueue->GetFileByID(ucharFileid);//
					if(pFile)
					{
		...
		case NOTES:
			{
				...
				if (pFromContact->GetVersion() >= 3/*47b*/)
				{
					...
					uchar ucharFileid[16];
					m_uTarget.ToByteArray(ucharFileid);
					CKnownFile* pFile = m_KadManager.pSharedFiles->GetFileByID(ucharFileid);//
					if(pFile)
					{
						m_pfileSearchTerms.WriteUInt64(pFile->GetFileSize());
						if (thePrefs.GetDebugClientKadUDPLevel() > 0)
		...
		case STOREFILE:
			{
				...
				if (pFile)
				{
					...
					if( m_KadManager.IsFirewalled() )//
					{
						...
						else if( m_KadManager.pClientList->GetBuddy() ) // We are firewalled, make sure we have a buddy.
						{
							...
							listTag.push_back(new CKadTagUInt(TAG_SERVERIP, m_KadManager.pClientList->GetBuddy()->GetIP()));
							listTag.push_back(new CKadTagUInt(TAG_SERVERPORT, m_KadManager.pClientList->GetBuddy()->GetUDPPort()));
...
void CSearch::ProcessResultNotes(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	bool bFlag = theApp.m_KadImpl.m_SearchList.AddNotes(pEntry, ucharFileid); //

	// Check if this hash is in our shared files..
	CAbstractFile* pFile = (CAbstractFile*)m_KadManager.pSharedFiles->GetFileByID(ucharFileid); //

	// If we didn't find a file in the shares check if it's in our download queue.
	if(!pFile)
		pFile =(CAbstractFile*)m_KadManager.pDownLoadQueue->GetFileByID(ucharFileid); //
...
//	theApp.emuledlg->kademliawnd->searchList->SearchRef(this); //DEL LIPING
	...
	theApp.m_KadImpl.m_SearchList.KademliaSearchKeyword(m_uSearchID, &uAnswer, sName, uSize, sType, uPublishInfo
