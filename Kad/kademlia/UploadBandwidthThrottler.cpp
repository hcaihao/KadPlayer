...
UploadBandwidthThrottler::UploadBandwidthThrottler(void) {
	...
	//DWORD id;
	//::CreateThread(NULL,FALSE,RunProcEx,this,FALSE,&id);
}
...
//ADD LIPING
DWORD WINAPI UploadBandwidthThrottler::RunProcEx(LPVOID pParam) 
{
	DbgSetThreadName("UploadBandwidthThrottler");
	UploadBandwidthThrottler* uploadBandwidthThrottler = (UploadBandwidthThrottler*)pParam;
...
UINT UploadBandwidthThrottler::RunInternal() 
{
	...
	while(doRun) {
		...
		//allowedDataRate = theApp.lastCommonRouteFinder->GetUpload(); //DEL LIPING
		allowedDataRate = 0;
...
#define TIME_BETWEEN_UPLOAD_LOOPS 1
        ...
        if(allowedDataRate != _UI32_MAX) 
		{
            ...
            if(timeSinceLastLoop == 0) 
			{
				...
				if(_I64_MAX/timeSinceLastLoop > allowedDataRate && _I64_MAX-allowedDataRate*timeSinceLastLoop > realBytesToSpend) 
			{
