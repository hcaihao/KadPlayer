...
#include "../QuickSearch/QuickSearchDlg.h"
#include "../file/SearchFile.h"
...
void CSearchList::RemoveResult(CSearchFile* todel)
{
	...
	for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
	{
		...
		if (cur_file == todel)
		{
			//theApp.emuledlg->searchwnd->RemoveResult(todel); //DEL LIPING
...
void CSearchList::NewSearch(CStringA strResultFileType, uint32 nSearchID, ESearchType eSearchType, CString strSearchExpression, bool bMobilMuleSearch)
{
	//	if (pWnd)
	//	outputwnd = pWnd;
	...
	uint32 nServerIP, uint16 nServerPort, bool* pbMoreResultsAvailable)
{
	...
	if (m_MobilMuleSearch)
		//theApp.mmserver->SearchFinished(false);//DEL LIPING
		m_MobilMuleSearch = false;
	...
	for (POSITION pos = m_listFileLists.GetHeadPosition(); pos != NULL;)
	{
		...
		for(POSITION pos2 = listCur->m_listSearchFiles.GetHeadPosition(); pos2 != NULL; )
		{
			...
			if (!md4cmp(hash, sf->GetFileHash())){
				m_KadManager.pDownLoadQueue->AddSearchToDownload(sf, 2, cat);
				return;
			}
		}
	}
...
bool CSearchList::AddToList(CSearchFile* toadd, bool bClientResponse, uint32 dwFromUDPServerIP)
{
	...
	for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
	{
			...
			&& md4cmp(parent->GetFileHash(), toadd->GetFileHash()) == 0)
		{
			...
			// 				outputwnd->UpdateSources(parent);//DEL LIPING
	...
	USES_CONVERSION;
	CString s = toadd->GetFileName();
	TRACE("文件名:[%s]\r\n",T2A(s.GetBuffer()));
	CQuickSearchDlg::GetInstance()->AddReuslt(toadd);
...
void CSearchList::AddResultCount(uint32 nSearchID, const uchar* hash, UINT nCount, bool bSpam)
{
	...
	if (m_KadManager.pSharedFiles->GetFileByID(hash) || m_KadManager.pDownLoadQueue->GetFileByID(hash))
		return;
...
void CSearchList::RecalculateSpamRatings(uint32 nSearchID, bool bExpectHigher, bool bExpectLower, bool bUpdate){
	...
	for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
	{
			...
			&& !(!pCurFile->IsConsideredSpam() && bExpectLower))
		{
			...
			//	outputwnd->UpdateSources(pCurFile);//DEL LIPING
		}
	}
}
...
void CSearchList::StoreSearches(){
	...
	try{
		...
		for (pos = m_listFileLists.GetHeadPosition(); pos != NULL; ){
			...
			//	if (theApp.emuledlg->searchwnd->GetSearchParamsBySearchID(pSl->m_nSearchID) != NULL)//DEL LIPING
			//	nCount++;
		}
	...
	try {
		...
		for (int i = 0; i < nCount; i++){
			...
			if (strResultType == ED2KFTSTR_PROGRAM)
				strResultType.Empty();
			NewSearch( strResultType, pParams->dwSearchID, pParams->eType, pParams->strExpression, false);
		...
		//theApp.emuledlg->searchwnd->SetNextSearchID(max(nHighestEd2kSearchID + 1, 0x80000000));//DEL LIPING
	}
	catch(CFileException* error){
		if (error->m_cause == CFileException::endOfFile)
...
bool CSearchList::GetSearchPacket(CSafeMemFile* pData, SSearchParams* pParams, bool bTargetSupports64Bit, bool* pbPacketUsing64Bit)
{
