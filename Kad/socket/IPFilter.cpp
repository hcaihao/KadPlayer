...
//static CIPFilterArray m_iplist;
...
void CIPFilter::AddIPRange(uint32 start, uint32 end, UINT level, const CStringA& rstrDesc) {
	m_iplist.Add(new SIPFilter(start, end, level, rstrDesc));
}

CString CIPFilter::GetDefaultFilePath() const
{
	return thePrefs.GetConfigPath() + DFLT_IPFILTER_FILENAME;
}
...
{
	CString strFilePath = thePrefs.GetConfigPath() + DFLT_IPFILTER_FILENAME;
